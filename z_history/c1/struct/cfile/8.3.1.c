...
void binsearch(int low,int high,int k,int *pos)
{int mid;
  
 mid=(low+high)/2;printf("(%d,%d,%d)",low,high,k);
if(low>high){*pos=-1;return;}
if(k==R[mid].key){*pos=mid;return;}
if(k<R[mid].key)binsearch(low,mid-1,k,pos);
else binsearch(mid+1,high,k,pos);
}

int main()
{   int i;char ch;int mykey,position;
...
while((ch=getchar())!='$')
{
   switch(ch)
   {
      ...
      case '4':printf("input key to find:");scanf("%d",&mykey);
binsearch(0,N-1,mykey,&position);printf("\nposition is R[%d]",position);
      case '\n':;
      case '$':break;
